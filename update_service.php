...
function insertItems() {
    ...
    if (isset($itemCode) || isset($company) || isset($wing) || isset($floor) || isset($loccode) || isset($maincat) || isset($subcat) || isset($itemname) || isset($suppler) || isset($assetcategory) || isset($quantity)) {
        ...
        if (strcmp($supplierName, "Select") == 0) {
          ...
          if (hasImageToSubCategory($itemCode)) {
          $imagepath = saveImage($item_img, $itemCode);
        ...
        $itm_query = "UPDATE [tblItemMaster] SET
	 ...
	 WHERE [ItemCode] = '$itemCode'";
        ...
        /*$itm_params = array( $itemname, $subcat, "NOS", $cost, $quantity, 0, "NOS",
            0, 0, 0, 0, 0, $loccode, "BOTH", "0", "nosuit", $printName, "N", "",
            $imagepath, 0, 0, 0, 0, 0, 0, 0, "LKR", 0, 0, 1, $supplierCode, $supplierName,
            "p", 0, $condition, "2", "NO",$itemCode);*/
        //error_log($itm_query." \n",0);
        //error_log(print_r($itm_params,true),0);
       // echo $itm_query;
        $stmt = sqlsrv_query($conn, $itm_query);
        ...
        if ($imagepath != "null") { 
            ...
            $s_query = "UPDATE tblItemMaster  SET Picture = (?) WHERE ItemCode = (?)";
            $s_param = array($imagepath, $itemCode);
        ...
        $response["success"] = 1;
        $response["message"] = $itemCode. " detail Update Successfull.";
        header('Content-type: application/json');
        echo json_encode($response);
    } else {
        $response["success"] = 2;
 ...
 * @return bool
 */
function hasImageToSubCategory($itemCode) {
    $conn = local_connect();

    $hasImage = false;

    $query = "SELECT Picture FROM tblItemMaster WHERE Itemcode = (?)";
